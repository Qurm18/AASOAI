...
export class AudioEngine {
  ...
  public async initialize(audioElement: HTMLAudioElement, forceRenewSource: boolean = false) {
    ...
    const firstTime = this.filtersA.length === 0;
    if (firstTime) {
    ...
    // MANDATORY RE-CONNECTION (outside the firstTime block)
    ...
    // Routing Logic (Sync with setPhaseMode)
    if (firstTime) {
      // Connect to iirGain initially
    ...
    // TAP NODE
    if (this.tapNode) {
        try { this.analyzerFloat!.disconnect(); } catch (e) {}
  ...
  public async crossfade(newUrl: string, audioEl: HTMLAudioElement, seekTime?: number): Promise<void> {
    if (!this.masterGain || !this.context || this.isCrossfading) return;
  ...
  public getFloatFrequencyData(): Float32Array | null {
    ...
    this.analyzerFloat.getFloatFrequencyData(this.floatBuffer);
  ...
  public getCrestFactorDb(): number {
    ...
    this.analyzerFloat.getFloatTimeDomainData(this.timeDomainBuffer);
  ...
  public getAdaptiveFeatures(): AdaptiveFeatures | null {
    ...
    this.analyzerFloat.getFloatFrequencyData(floatBuffer);
    this.analyzerFloat.getFloatTimeDomainData(timeDomainBuffer);
    ...
    const job = (async () => {
      try {
        ...
        const result: Record<string, { time: number; type: 'high' | 'low' | 'neutral' }> = {
          sub_bass:        { time: await peak(0), type: 'high' },
          bass_depth:      { time: await peak(1), type: 'high' },
          mid_punch:       { time: await peak(2), type: 'high' },
          warmth_body:     { time: await peak(3), type: 'low' },
          vocal_clarity:   { time: await peak(4), type: 'low' },
          instrument_sep:  { time: await peak(5), type: 'low' },
          presence:        { time: await peak(6), type: 'low' },
          high_frequency:  { time: await peak(7), type: 'neutral' },
          sibilance:       { time: await peak(8), type: 'neutral' },
          overall_balance: { time: dur * 0.50, type: 'neutral' },
          sub_mid_blend:   { time: await peak(1), type: 'high' },
          high_air_only:   { time: await peak(9), type: 'neutral' },
          warmth_no_mud:   { time: await peak(3), type: 'low' },
          presence_no_harshness: { time: await peak(6), type: 'low' },
        };
